...
// 2007-03-21
//
// Copyright (c) 2007 Patrick Mueller, pmuellr@yahoo.com
// Copyright (c) 2007 Josh Staiger, josh@joshstaiger.org
//
...
function getItemAvailabilityDiv() {
	...
	document.body.insertBefore(itemAvailabilityDiv, document.body.firstChild)

	return itemAvailabilityDiv
}

function getItemAvailabilityDivOld() {
...
function otherEditionSearch(isbn, fn) {
    ...
    GM_xmlhttpRequest ({
        ...
        onload:function(results) {
	    	...
	    	var page            = results.responseText;
	    	var beginResultsPos = page.search(/<a name="other_editions"\/>/);

	    	if (beginResultsPos == -1) {
				throw new Error('Could not find other_editions anchor text');
	    	}

	    	var endResultsPos = page.slice(beginResultsPos).search(/<\/div>/) + beginResultsPos;
	    	var itemReg       = /a href=.*amazon.com.*(\d{10})/g;
	    	...
	    	while((match = itemReg.exec(page)) && itemReg.lastIndex <= endResultsPos) {
				pageIsbns.push(match[1]);
	    	}

	    	if (pageIsbns.length == 0) {
...
// Return true if we are browsing an "All Editions" page
 ...
 function isAllEditionsPage() {
    return window.location.href.search('other-editions') != -1;
}


function getOtherEditionsAnchorElement() {
    var otherEditionsAnchorElement = xpathFirst("//a[@name='other_editions']");
    
    if (!otherEditionsAnchorElement) {
        throw new Error("Other Editions anchor not found");
    }

    return otherEditionsAnchorElement;
    
}

function getOtherEditionsTableElement() {
    var nextSiblingElement = getOtherEditionsAnchorElement().nextSibling;

    if (nextSiblingElement.tagName != 'TABLE') {
        throw new Error("Other editions table was not next sibling of otherEditionsAncor");
    }

    return nextSiblingElement;
}

function getOtherEditionRowElements() {
    var rowElementsArray = getOtherEditionsTableElement().getElementsByTagName("tr");
    
    if( rowElementsArray.length == 0 ) {
        throw new Error("Other Editions table does not contain any rows");
    }

    return rowElementsArray;
}

function createLibraryEditionHeaderColumn() {
    var tdElement = document.createElement('td');
    tdElement.className = "tiny";
    
    tdElement.innerHTML = "@ " + libraryName + "?";
    return tdElement;
}

function addLibraryEditionHeaderColumn(rowElements) {
    if(rowElements == null) {
        throw new Error("rowElement cannot be null");
    }
    
    if(rowElements.length < 1) {
        throw new Error("rowElements cannot be empty");
    }

    rowElements[1].insertBefore(createLibraryEditionHeaderColumn(), rowElements[1].getElementsByTagName('td')[1]);
}

function getRowEditionIsbn(rowElement) {
    if(rowElement == null) {
        throw new Error("rowElement cannot be null");
    }

    var match = rowElement.getElementsByTagName('td')[0].getElementsByTagName('a')[0].href.match(/\/(\d{7,9}[\d|X])\//);;

    if(!match) {
        throw new Error("did not match edition isbn");
    }

    return match[1];
}

function createLibraryEditionLinkColumn(libraryUri) {
    if(libraryUri == null) {
        throw new Error("libraryUri cannot be null");
    }

    var tdElement = document.createElement('td');
    var aElement = document.createElement('a');
    aElement.href = libraryUri;
    aElement.innerHTML = "yes!";
...
function insertLibraryLinkColumn(rowElement, libraryUri) {
    if(rowElement == null) {
        throw new Error("rowElement cannot be null");
    }
    ...
    rowElement.insertBefore(createLibraryEditionLinkColumn(libraryUri), rowElement.getElementsByTagName('td')[1]);
}

function createUnavailableColumn() {
    return document.createElement('td');
}

function insertUnavailableColumn(rowElement) {
    if(rowElement == null) {
        throw new Error("rowElement cannot be null");
    }

    rowElement.insertBefore(createUnavailableColumn(), rowElement.getElementsByTagName('td')[1]);
}



function insertEditionCheckColumn(rowElement) {
    if(rowElement == null) {
        throw new Error("rowElement cannot be null");
    }

    libSearch( getRowEditionIsbn(rowElement), function(libraryUri) {
                   if(libraryUri) {
                       insertLibraryLinkColumn(rowElement, libraryUri);
                       return;
                   }
                   
                   insertUnavailableColumn(rowElement);
    });
...
function searchAllEditions() {
    addLibraryEditionHeaderColumn(getOtherEditionRowElements());
    
    for(var x = 2; x<getOtherEditionRowElements().length; x++) {
        insertEditionCheckColumn(getOtherEditionRowElements()[x]);
    }
}
...
addGlobalStyle('.stub { height: 1em; margin: 1em; } .libraryAvailable, .libraryUnavailable { font-weight:bold; color:rgb(153,0,0); } ');

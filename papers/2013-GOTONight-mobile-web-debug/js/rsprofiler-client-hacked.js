...
document.addEventListener('DOMContentLoaded', cbLoaded, false)

//-----------------------------------------------------------------------------
function cbLoaded() {
    if (!window.ProfileData) {
        $("#profiles-chart").html("profile data is not available :-(")
        return
    }

    showProfileChart(window.ProfileData)
}
...
function showProfileChart(profile) {
    ...
    .enter().append("svg:g")
        .attr("transform", function(d) {
            return "translate(" + x(d.x) + "," + (h - y(d.y) - barHeight) + ")";
        })
        .on("click",     click)
